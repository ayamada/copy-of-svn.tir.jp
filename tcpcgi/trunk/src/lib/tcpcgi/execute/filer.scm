...
;;; ToDo : nph形式に直す事
;;; ToDo : error-procスロットを廃止し、parameterのtcpcgi経由で、
;;;        (ref (with-module tcpcgi (tcpcgi)) 'errordoc-instance)
;;;        のようにアクセスするようにする
...
(define-module tcpcgi.execute.filer
  ...
  (use tcpcgi.errordoc)
  (export
    <tcpcgi.execute.filer>
    make-filer-thunk
    *mime-table*
    ))
(select-module tcpcgi.execute.filer)


(define-class <tcpcgi.execute.filer> ()
  (
