...
;;; ToDo : 以下の三つが完成したら、0.3としてreleaseする予定。

;;; ToDo : vhost対応
;;; ToDo : errordoc指定は、ファイルパスも可能とする
;;; ToDo : 巨大ファイルをfilerから返す時のメモリ消費を抑えたい
;;;        しかし、そうするには、今の、レスポンス全部を
;;;        一旦メモリに持つ方式では無理。ちょっと作り直しが必要。
;;;        - ファイル出力、nph時は、execute時は、lazyフラグを立て、
;;;          他の必要な情報をセットはするものの、まだcgi実行は行わない。
;;;          ロギングが完了し、出力する段階で、結果を直に実行するようにする。
    ...
    (let ((old-log-port-mode #f)
      ...
      (dynamic-wind
        ...
        (lambda ()
          ...
          (set! (port-buffering (cgi-error-port-of self)) old-error-port-mode)
          ))))
