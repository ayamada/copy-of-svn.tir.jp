...
;;; ToDo : 以下の二つ（リダイレクトと遅延実行）の完了で次のバージョンとする
...
;;;        他にも、selectで実装した方が良い部分は多いので（主にGC関連）、
;;;        後でselectに変更可能な部分はselectに直すようにする。
...
(define-class <tcpcgi> ()
  (
   ...
   ;; request uriがincomplete stringで送られてきた場合に変換するフラグ
   (convert-incomplete-string-uri :accessor convert-incomplete-string-uri-of
...
(define (convert-request-line! state)
  (or
    (and-let* ((request-line (ref state 'request-line))
               (ces (ces-guess-from-string request-line "*JP")))
      (set!
        (ref state 'request-line)
        (ces-convert request-line ces)))
    (begin
      (set! (ref state 'response-code) 400)
      (set! (ref state 'internal-description) 'cannot_understand_request_uri)
      #f)))
    ...
    (with-timer-handler
      ...
      (lambda ()
        ...
        (and
          ...
          ;; request-lineがincomplete-stringなら、変換を行う
          (when (and
                  (convert-incomplete-string-uri-of self)
                  (string-incomplete? (ref state 'request-line)))
            (convert-request-line! state))
      ...
      `(("date" ,(sys-strftime
        ...
        ,@(or
            (and
              ...
              (not (assoc "content-length" src-alist))
              `(("content-length" ,(string-size src-body))))
            '())
      ...
      (receive (response-code
        ...
        (cond
          ...
          (location (let* ((true-location
                           ...
                           (true-content-length (string-size
                                                  true-response-body))
                           )
                      (values
                        302
