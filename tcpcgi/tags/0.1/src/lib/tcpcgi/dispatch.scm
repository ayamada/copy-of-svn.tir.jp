...
(define-class <tcpcgi.dispatch.none> ()
  (
   (cgi-thunk :accessor cgi-thunk-of
              :init-keyword :cgi-thunk
              :init-value #f)
   (nph :accessor nph-of
        :init-keyword :nph
        :init-value #f)
...
;; ToDo :
(define (make-filer path . keywords) #f) ; dummy
;; cgiとして文字列が渡された場合は、
;; ファイル表示cgi-thunkを内部で生成してセットするようにする
  ...
  (and
    ...
    (let* ((param-is-list? (list? param))
           ...
           (keywords (if param-is-list?
                         (cdr param)
                         '()))
           (cgi-thunk (if (string? cgi-target)
                        (apply make-filer cgi-target keywords)
                        cgi-target))
           )
