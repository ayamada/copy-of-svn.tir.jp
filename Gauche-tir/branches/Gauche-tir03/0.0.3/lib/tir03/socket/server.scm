...
;;;   具体的には、readした結果が#<eof>だったり、シグナル(SIGTERM)が流れたり、
...
(define session->select-handler cadddr)
...
(define (make-server-accept-handler self server-socket)
  (lambda (fd flag)
    (let ((client-socket (socket-accept server-socket))
      ...
      (let1 select-handler (lambda (in flag)
                             (verbosef self "received data ccid ~s" ccid)
        ...
        (selector-add! (selector-of self)
                       ...
                       '(r))
        ;; session-tableに登録
        (hash-table-put! (session-table-of self)
                         ccid
                         (make-session
