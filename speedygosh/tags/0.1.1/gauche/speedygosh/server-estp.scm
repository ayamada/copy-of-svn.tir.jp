...
(define env-path (make-parameter #f))
...
(define (with-env env argv thunk)
  ;; NB: speedygoshコマンドがサーバを起動する段階で、
  ;;     既に環境変数は、PATH以外全てclearenvされているものとする。
  (dynamic-wind
    (lambda ()
      ;; 新しい環境変数をセットする
      (for-each
    ...
    (lambda ()
      ;; セットした環境変数を消す
      ;; 但し、PATHだけは復元する
      ;; (speedygosh-termination-handlersが使う可能性がある為)
      (for-each
        (lambda (name+val)
          (sys-unsetenv (car name+val)))
        env)
      (sys-setenv "PATH" (env-path) #t))))
