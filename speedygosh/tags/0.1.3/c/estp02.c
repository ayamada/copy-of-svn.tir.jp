...
int estp02_comm (char *sock_path, int argc, char *argv[], char **env) {
  ...
  int old_stdin_fcntl = fcntl(STDIN_FILENO, F_GETFL);
  int old_sock_fcntl  = fcntl(sock_fd, F_GETFL);
  fcntl(STDIN_FILENO, F_SETFL, old_stdin_fcntl | O_NONBLOCK);
  fcntl(sock_fd,      F_SETFL, old_sock_fcntl  | O_NONBLOCK);
  /* stdin/stdoutとソケットを結びつける */
  interconnect_stream(sock_in, sock_out, sock_fd);
  /* 一応、ブロックモードを戻しておく */
  fcntl(STDIN_FILENO, F_SETFL, old_stdin_fcntl);
  fcntl(sock_fd,      F_SETFL, old_sock_fcntl);
