...
(define-method irc-send-enqueue! ((self <pib>) event)
  ;; 送信キューは、この時点で一旦変換を行い、正規性を確認する
  (let1 converted-message (ces-convert (event->message event #t)
                                       (gauche-character-encoding)
                                       (irc-server-encoding-of self))
    ;; 文字列に展開した後のIRCリクエストが最大510文字+CRLFを
    ;; 越えていない事を確認する
    (when (<= 512 (string-size converted-message))
      (error "message too long" event))
    ;; キューにeventをつっこむ
    (with-locking-mutex
      (irc-send-queue-mutex-of self)
      (lambda ()
...
(define-method %irc-send-thread ((self <pib>))
  (let loop ()
    ...
    (let* ((event (irc-send-dequeue! self))
           (message (and
      ...
      (let/cc return
        (let* ((command (event->command event))
               (channel-dir (guard (e (else global-dirname))
                              (let ((channel (or (event->channel event) "")))
                                ;; 特定のcommandなら、ロギング自体を行わない
                                ;(when (equal? command "PASS") (return))
                                ;; commandが数値なら、channelとは見做さない
                                (when (number? command)
                                  (error "!"))
                                ;; 特定のcommandなら、channelとは見做さない
                                (when (equal? command "USER")
                                  (error "!"))
                                (when (equal? command "PASS")
                                  (error "!"))
                                ...
                                (when (equal? command "QUIT")
                                  (error "!"))
                                (when (equal? command "ERROR")
                                  (error "!"))
                                (when (equal? command "INVITE")
                                  (error "!"))
                                ;; ユーザ間通信なら、channelではなく、
                                ;; prefixから取得する
                                (when (and
                                        (or
                                          (equal? command "PRIVMSG")
                                          (equal? command "NOTICE"))
                                        (equal? channel (get-current-nick))
                                        (event->prefix event))
                                  (set! channel
