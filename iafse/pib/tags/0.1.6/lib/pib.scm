    ...
    (let1 command (event->command event)
      ...
      (let* ((params (event->params event))
        ...
        (when (equal? channel "AUTH")
          (return #f))
        ;; channelがチャンネル名書式(#または&はじまり)なら、
        ;; このchannelが返信先になる
        (when (#/^(?:\#|\&)/ channel)
          (return channel))
        (let1 prefix (event->prefix event)
          ;; prefixが#f、つまり送信イベントなら、channelは自分以外の誰か
          ;; という事になるので、それをそのまま返す
          (unless prefix
            (return channel))
          ;; 受信イベントだった。つまり、このeventは自分宛てだった。
          ;; TODO: この部分の解釈が少し怪しい
          ;;       (とりあえずPRIVMSGとNOTICEはこれでいいが、
          ;;        それ以外のcommandで返信先はprefixではなくchannelに
          ;;        なるものが存在する可能性がある)
          ;; よって、prefixから、このmessageの送信元を割り出し、それを返す
          ;; prefixの書式は、"nick!~username@hostname"
          ;; よって、!より前の部分を取り出すだけで良い
          (and-let* ((matched (#/\!/ prefix)))
            (matched 'before)))))))
